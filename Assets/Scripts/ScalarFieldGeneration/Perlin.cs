...
public static class Perlin
{
    ...
    public static Vector3 FlowField(Vector3 point)
    {
        float val = Noise(point) + 1;
        Vector3 direction = new Vector3(Mathf.Sin(val * Mathf.PI), Mathf.Cos(val * Mathf.PI), Mathf.Sin(val * Mathf.PI));
        return direction;
    }
    ...
    public static float EvaluateFBM(Vector3 point,
                  float amplitude, float frequency,
                  int octaveCount, float persistence, float lacunarity)
    {

        float value = 0;
        Vector3 offset = new Vector3(1002,1400.1f,25010);
        for (int i = 0; i < octaveCount; i++)
        {
            value += amplitude * Noise(point * frequency+ offset);
